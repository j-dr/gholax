...
class Boltzmann(LikelihoodModule):
    ...
    def compute(self, state, params_values):
        ...
        boltz_params = {
            ...
            "P_k_max_h/Mpc": 20.0,
            "z_pk": "0.0,10",
            "A_s": float(params_values["As"]) * 1e-9,
            "n_s": float(params_values["ns"]),
            "h": float(params_values["H0"]) / 100,
            "N_ur": 0.00641,
            "N_ncdm": 1,
            "deg_ncdm": 3,
            "Omega_Lambda": 0.0,
            "w0_fld": float(params_values["w"]),
            "wa_fld": 0.0,
            "m_ncdm": float(params_values["mnu"]) / 3,
            "tau_reio": 0.0568,
            "omega_b": float(params_values["ombh2"]),
            "omega_cdm": float(params_values["omch2"]),
