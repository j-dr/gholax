...
from jaxeft.util.model import Model
...
if __name__ == '__main__':
    ...
    z = model.likelihoods['RSDPK'].likelihood_pipeline[3].z
    ...
    Ptrain_all, Ftrain_all = load_training_data('RSDPK.sigma8_z', training_data)
    ...
    try:
        training_data.create_dataset('params_sigma8z', Ptrain.shape)
    except:
        del training_data['params_sigma8z']
        training_data.create_dataset('params_sigma8z', Ptrain.shape)

    training_data['params_sigma8z'][:] = Ptrain[:]        
    
    try:
        training_data.create_dataset('sigma8z', Ftrain.shape)
    except Exception as e:
        print(e)
        del training_data['sigma8z']
        training_data.create_dataset('sigma8z', Ftrain.shape)
    training_data['sigma8z'][:] = Ftrain
