...
if __name__ == '__main__':
    ...
    training_data = h5py.File(training_data_filename, 'r+')
    z = model.likelihoods['Nx2PTAngularPowerSpectrum'].likelihood_pipeline[6].z
    k =  model.likelihoods['Nx2PTAngularPowerSpectrum'].likelihood_pipeline[6].k
    nspec = 19
    Ptrain_all, Ftrain_all_spec = load_training_data('Nx2PTAngularPowerSpectrum.p_ij_real_space_bias_grid', training_data)
    
    rescale_by_s8sq = True
    

    Ftrain_all = Ftrain_all_spec[:,:,:,:] #izsk
    ...
    try:
        training_data.create_dataset(f'p_cleft', Ftrain.shape)
    except:
        del training_data[f'p_cleft']
        training_data.create_dataset(f'p_cleft', Ftrain.shape)

    training_data[f'p_cleft'][:] = Ftrain
    del Ftrain
