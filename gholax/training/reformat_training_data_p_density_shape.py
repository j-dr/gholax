...
if __name__ == '__main__':
    ...
    training_data = h5py.File(training_data_filename, 'r+')
    z = model.likelihoods['Nx2PTAngularPowerSpectrum'].likelihood_pipeline[9].z
    k =  model.likelihoods['Nx2PTAngularPowerSpectrum'].likelihood_pipeline[9].k
    nspec = 21
    Ptrain_all, Ftrain_all_spec = load_training_data('Nx2PTAngularPowerSpectrum.p_ij_real_space_density_shape_grid', training_data)
    rescale_by_s8sq = True

    Ftrain_all = Ftrain_all_spec[:,:,:,:] #iskz
    _, sigma8_z_all = load_training_data('Nx2PTAngularPowerSpectrum.sigma8_z', training_data)
    ...
    try:
        training_data.create_dataset(f'p_density_shape', Ftrain.shape)
    except:
        del training_data[f'p_density_shape']
        training_data.create_dataset(f'p_density_shape', Ftrain.shape)

    training_data[f'p_density_shape'][:] = Ftrain
    del Ftrain
