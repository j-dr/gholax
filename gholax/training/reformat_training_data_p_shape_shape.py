...
if __name__ == '__main__':
    ...
    training_data = h5py.File(training_data_filename, 'r+')
    z = model.likelihoods['Nx2PTAngularPowerSpectrum'].likelihood_pipeline[10].z
    k =  model.likelihoods['Nx2PTAngularPowerSpectrum'].likelihood_pipeline[10].k
    nspec = 13
    nm = 3
    Ptrain_all, Ftrain_all_spec = load_training_data('Nx2PTAngularPowerSpectrum.p_mij_real_space_shape_shape_grid', training_data)
    ...
    for m in range(nm):
        ...
        try:
            training_data.create_dataset(f'p{m}_shape_shape', Ftrain.shape)
        except:
            del training_data[f'p{m}_shape_shape']
            training_data.create_dataset(f'p{m}_shape_shape', Ftrain.shape)

        training_data[f'p{m}_shape_shape'][:] = Ftrain
        del Ftrain
